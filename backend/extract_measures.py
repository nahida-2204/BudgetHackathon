...
# ==========================================
def extract_sector_measures(
    sector_name: str, search_query: str, year_filter: str
) -> str:
    ...
    json_schema = '{"sectors": [{"sector": "sectorName", "measures": [{"title": "title", "description": "description"}]}]}'
    ...
    Instructions:
    - Return the actual measures in the following json format: {json_schema}
    - Replace "sectorName" with the actual sector name.
...
if __name__ == "__main__":
    ...
    for sector, query in sector_queries.items():
        ...
        measures_text = extract_sector_measures(
            sector_name=sector, search_query=query, year_filter=target_year
        )
